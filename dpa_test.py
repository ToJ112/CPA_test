...
if __name__ == "__main__":
    ...
    # c = np.einsum("dt,dt->t", a, a, optimize='optimal')
    d = np.multiply(a, a).sum(axis=0)
    # diff_t = a - (np.einsum("dt->t", a, optimize='optimal') / np.double(D))
    # diff_h = b - (np.einsum("dk->k", b, optimize='optimal') / np.double(D))
    # cov = np.einsum("dk,dt->kt", diff_h, diff_t, optimize='optimal')  # Covariant.
    # std_h = np.einsum("dk,dk->k", diff_h, diff_h, optimize='optimal')  # Standard deviation of h.
    # std_t = np.einsum("dt,dt->t", diff_t, diff_t, optimize='optimal')  # Standard deviation of t.
    diff_t = a - (np.einsum("dt->t", a, optimize='optimal') / np.double(6))
    time1 = time.time()
    diff_b = b - (np.einsum("dk->k", b, optimize='optimal') / np.double(4))
    std_h = np.einsum("dk,dk->k", diff_b, diff_b, optimize='optimal')  # Standard deviation of h
    print(std_h)
    time2 = time.time()
    print(np.std(b, axis=0))
    time3 = time.time()
    print(((time2 - time1)-(time3 - time2))>0)
